import {filteredOffers, comparison, freshness, safeFlightLink} from './model.mjs';
...
function renderSummary() {
  ...
  $('offer-timestamp').textContent=data.offers_as_of?`Preisstand: ${when(data.offers_as_of)} · Berlin`:'Noch keine geprüften Preise';
}
...
function renderOffers() {
  ...
  for(const q of offers) {
    ...
    const verdict=node('td');verdict.append(node('span',c.verdict,`pill ${c.verdict==='Kauf prüfen'?'good':c.verdict==='Beobachten'?'watch':'mid'}`));
...
function renderChart() {
  ...
  $('chart-note').textContent=`${points.length} Messungen${points.length?' seit '+when(points[0].at):''}. ${c.low===null?'Noch kein früherer Vergleichspreis.':'Bisheriges Tief vor dieser Messung: '+euro(c.low)+'.'} Gleiche Reisedaten und Flugart, ggf. andere Airline.`;
...
async function load() {
  ...
  try {
    ...
    data=value;fillSelect('origin',data.config.origins,v=>({DUS:'Düsseldorf (DUS)',FRA:'Frankfurt (FRA)',AMS:'Amsterdam (AMS)'})[v]||v);
    fillSelect('departure',[...new Set(data.offers.map(q=>q.departure))].sort(),day);
    fillSelect('days',Array.from({length:data.config.max_trip_days-data.config.min_trip_days+1},(_,i)=>data.config.min_trip_days+i),v=>`${v} Tage`);
