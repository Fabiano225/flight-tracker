...
def export_data(db_path, config, now=None):
    ...
    try:
        ...
        if by_run:
            offers = max(by_run.values(), key=lambda items: items[0]['at'])
            result['offers'] = sorted(offers, key=lambda q: (q['price'], q['origin'], q['departure']))
            result['offers_as_of'] = offers[0]['at']
        return result
    finally:
        db.close()
...
def build(db_path, output, config_path=ROOT / 'config.json'):
    ...
    data = export_data(db_path, Config.load(config_path))
